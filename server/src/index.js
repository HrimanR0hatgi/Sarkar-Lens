...
app.post('/api/checklist',async(req,res)=>{ try{ const facts=req.body.facts ? FactsSchema.parse(req.body.facts) : await parseSituation(req.body.input||''); const services=selectServices(facts,getServices()); const sessionId=crypto.randomUUID(); res.json({sessionId,facts,services}); }catch(e){res.status(422).json({error:'Insufficient or ambiguous information.',details:e.message});} });
app.get('/api/services/:id',(req,res)=>{ const s=getServices().find(x=>x.id===req.params.id); if(!s)return res.status(404).json({error:'Service not found.'}); res.json(s); });
